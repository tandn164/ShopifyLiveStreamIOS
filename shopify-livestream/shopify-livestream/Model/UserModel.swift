...
class User: NSObject, NSCoding, Codable {
    ...
    init(dictionary: [String: Any]) {
        ...
        if let carts = dictionary["cart"] as? [[String: Any]] {
            var cartss: [CartModel] = []
            for cart in carts {
                cartss.append(CartModel(dictionary: cart))
            }
            self.cart = cartss
        }
        if let address = dictionary["address"] as? [String: Any] {
            self.address = AddressModel(dictionary: address)
        }
    }
